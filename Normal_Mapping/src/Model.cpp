...
std::unique_ptr<Mesh> Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (int type = aiTextureType_DIFFUSE; type <= aiTextureType_HEIGHT; ++type) {
...
std::vector<std::shared_ptr<Texture>> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, const std::string& typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) 
	{
		...
		if (loadedTextures.find(path) == loadedTextures.end()) 
		{
			...
			if (type == aiTextureType_SPECULAR)     texType = TextureType::Specular;
			if (type == aiTextureType_NORMALS)      texType = TextureType::Normal;
			if (type == aiTextureType_HEIGHT)       texType = TextureType::Height;
