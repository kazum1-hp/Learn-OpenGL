...
FrameBuffer::FrameBuffer(Window& window, bool useDepth, bool useMs, bool useDepthMap)
	: width(window.getWidth()), height(window.getHeight())
{
	init(width, height, useDepth, useMs, useDepthMap);
}

FrameBuffer::FrameBuffer(int w, int h, bool useDepth, bool useMs, bool useDepthMap)
	: width(w), height(h)
{
	init(width, height, useDepth, useMs, useDepthMap);
}

void FrameBuffer::init(int w, int h, bool useDepth, bool useMs, bool useDepthMap)
{
	...
	else
	{
		...
		if (useDepthMap)
		{
			...
			// if only need depth£¨ex: shadow mapping£©
			glDrawBuffer(GL_NONE);
			glReadBuffer(GL_NONE);

			depthTexture = std::make_shared<Texture>(texDepth, 1);
		}
