     ...
     dirLight(glm::vec3(1.0f), glm::vec3(-2.2f, -2.0f, -2.3f), LightType::Directional)
{
    ...
    pointLights.push_back(Light(glm::vec3(5.0f, 0.0f, 0.0f), glm::vec3(-4.0f, 0.5f, -3.0f), LightType::Point));
    pointLights.push_back(Light(glm::vec3(0.0f, 0.0f, 14.0f), glm::vec3(3.0f, 0.5f, 1.0f), LightType::Point));
    ...
    shaders.push_back(std::make_unique<Shader>("shader/framebuffer.vs", "shader/framebuffer.fs"));    // scene framebuffer shader
    shaders.push_back(std::make_unique<Shader>("shader/skybox.vs", "shader/skybox.fs"));    // skybox shader
    shaders.push_back(std::make_unique<Shader>("shader/shadow.vs", "shader/shadow.fs"));    // dir shadow shader
    shaders.push_back(std::make_unique<Shader>("shader/pointShadow.vs", "shader/pointShadow.fs", "shader/pointShadow.gs"));    // point shadow shader
    shaders.push_back(std::make_unique<Shader>("shader/bloomBlur.vs", "shader/bloomBlur.fs"));    // bloomBlur shader
    shaders.push_back(std::make_unique<Shader>("shader/gBuffer.vs", "shader/gBuffer.fs"));      // gBuffer shader
    shaders.push_back(std::make_unique<Shader>("shader/lightPass.vs", "shader/lightPass.fs"));      // lightPass shader
    shaders.push_back(std::make_unique<Shader>("shader/debug.vs", "shader/debug.fs"));      // debug shader
    shaders.push_back(std::make_unique<Shader>("shader/drawDebug.vs", "shader/drawDebug.fs"));      // gbuffer debug shader
...
void Renderer::forwardPass()
{
    ...
    for (int i = 0; i < pointLights.size(); i++)
    {
        ...
        LightShader.setUniform("lightColor", pointLights[i].getColor());
        LightShader.setUniform("enabled", pointLights[i].lightOn() && input.isPointLightOn());
        if (pointLights[i].lightOn() && input.isPointLightOn())
        {
            cube.draw();
        }
    }
...
void Renderer::geometryPass()
{
    ...
    Shader& deferredShader = *shaders[7];
    deferredShader.use();
    // uniform setting
    deferredShader.setUniform("view", camera.getViewMatrix());
    deferredShader.setUniform("projection", camera.getProjectionMatrix());
    deferredShader.setUniform("viewPos", camera.getPosition());

    drawMesh(plane, deferredShader);
    deferredShader.setUniform("model", model);
    deferredShader.setUniform("hasNormalMap", false);
    deferredShader.setUniform("hasHeightMap", false);
    plane.draw();

    drawModel(baseball, deferredShader);
    deferredShader.setUniform("hasNormalMap", hasNormal);
    deferredShader.setUniform("hasHeightMap", hasHeight);
    deferredShader.setUniform("height_scale", height_scale);
    renderModel(transform, baseball, deferredShader);
...
void Renderer::lightPass()
{
    ...
    // paralleLight
    lightPassShader.setUniform("parallelLight.ambient", dirLight.getAmbient() * dirLight.getColor());
    lightPassShader.setUniform("parallelLight.diffuse", dirLight.getDiffuse() * dirLight.getColor());
    lightPassShader.setUniform("parallelLight.specular", dirLight.getSpecular() * dirLight.getColor());
    lightPassShader.setUniform("parallelLight.direction", dirLight.getLightDir());
    ...
    for (int i = 0; i < pointLights.size(); i++)
    {
        ...
        lightPassShader.setUniform(base + ".ambient", pointLights[i].getAmbient() * pointLights[i].getColor());
        lightPassShader.setUniform(base + ".diffuse", pointLights[i].getDiffuse() * pointLights[i].getColor());
        lightPassShader.setUniform(base + ".specular", pointLights[i].getSpecular() * pointLights[i].getColor());
        lightPassShader.setUniform(base + ".position", pointLights[i].getLightPos());
        lightPassShader.setUniform(base + ".enabled", pointLights[i].lightOn() && input.isPointLightOn());
        lightPassShader.setUniform("far_plane", pointLights[i].getFar());
    ...
    auto drawDebug = [&](GLuint tex, int index)
        {
            ...
            debugShader.setUniform("debugTex", 0);
            debugShader.setUniform("u_DebugMode", index);
            debugShader.setUniform("fov", camera.getFov());
            debugShader.setUniform("nearPlane", camera.getNearPlane());
            debugShader.setUniform("farPlane", camera.getFarPlane());
