...
FrameBuffer::FrameBuffer(Window& window, bool useDepth, bool useMs, bool useDepthMap2D, bool useDepthCube, bool useHdr, int colorCount)
	: width(window.getWidth()), height(window.getHeight()),
      m_useDepth(useDepth), m_useMs(useMs), m_useDepthMap2D(useDepthMap2D), m_useDepthCube(useDepthCube), m_useHdr(useHdr), m_colorCount(colorCount)
{
	init(width, height);
}

FrameBuffer::FrameBuffer(unsigned int w, unsigned int h, bool useDepth, bool useMs, bool useDepthMap2D, bool useDepthCube, bool useHdr, int colorCount)
	: width(w), height(h),
      m_useDepth(useDepth), m_useMs(useMs), m_useDepthMap2D(useDepthMap2D), m_useDepthCube(useDepthCube), m_useHdr(useHdr), m_colorCount(colorCount)
{
	init(width, height);
