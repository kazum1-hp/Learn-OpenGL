...
vec3 CalParallelLight(ParallelLight parallelLight, vec3 norm, vec3 viewDir, vec3 parallelLightDir, vec3 texColor, float parallelShadow, float ao, float roughness, float metallic);
...
void main()
{
	...
	vec3 parallelColor = CalParallelLight(parallelLight, norm, viewDir, parallelLightDir, texColor.rgb, parallelShadow, ao, roughness, metallic);
...
vec3 CalParallelLight(ParallelLight parallelLight, vec3 norm, vec3 viewDir, vec3 parallelLightDir, vec3 texColor, float parallelShadow, float ao, float roughness, float metallic)
{
    ...
    vec3 parallelLightColor = parallelLight.enabled? (ambient + (1 - parallelShadow) * Lo) : vec3(0.0);
...
vec3 CalPointLight(PointLight pointLight, vec3 norm, vec3 viewDir, vec3 pointLightDir, vec3 texColor, float pointShadow, float ao, float roughness, float metallic)
{
    ...
    vec3 radiance = texColor * attenuation;
    ...
    vec3 pointLightColor = pointLight.enabled? (ambient + (1 - pointShadow) * Lo) : vec3(0.0);
