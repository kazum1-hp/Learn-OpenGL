...
void InputManager::cursorState(GLFWwindow* window)
{
	...
	{
		if (!mouseOverImGui && cursorVisible)
		{
			glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
			cursorVisible = false;
		}
		else if (mouseOverImGui)
		{
			glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
			cursorVisible = true;
		}
...
{
	ImGui::SliderFloat("Move Speed", &moveSpeed, 10.0f, 100.0f);
}