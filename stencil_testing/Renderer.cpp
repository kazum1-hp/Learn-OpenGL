     ...
     material()
{
    ...
    shaders.push_back(std::make_unique<Shader>("outline.vs", "outline.fs")); // outline shader
...
{
    Shader& shader = *shaders[0];
    Shader& outlineShader = *shaders[1];
    ...
    outlineShader.setUniform("outlineThickness", static_cast<float>(0.5f * sin(glfwGetTime() * 5.0f) + 0.5f) * 0.15f);
    outlineShader.setUniform("outlineColor", glm::vec3(0.3f, 0.2f, 0.5f));
    //outlineShader.setUniform("time", static_cast<float>(glfwGetTime()));
