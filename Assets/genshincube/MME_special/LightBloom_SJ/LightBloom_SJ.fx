...
float mBloomColor6thHP : CONTROLOBJECT<string name="LightBloomController.pmx"; string item = "BloomColor6thH+";>;
float mBloomColor6thSP : CONTROLOBJECT<string name="LightBloomController.pmx"; string item = "BloomColor6thS+";>;
float mBloomColor6thVP : CONTROLOBJECT<string name="LightBloomController.pmx"; string item = "BloomColor6thV+";>;
float mBloomColor6thVM : CONTROLOBJECT<string name="LightBloomController.pmx"; string item = "BloomColor6thV-";>;
float DirtIntensity : CONTROLOBJECT<string name="LightBloomController.pmx"; string item = "DirtmapV+";>;
...
static float2 BloomOffset6 = ViewportOffset * 64;
...
static const float2 BloomOffsetX6 = float2(64 / BloomScale.x, 0.0);
...
static const float2 BloomOffsetY6 = float2(0.0, 64 / BloomScale.y);
...
static float3 mBloomColor6th = float3(mBloomColor6thHP, mBloomColor6thSP, lerp(lerp(mBloomIntensityMin, mBloomIntensityMax, mBloomColor6thVP), 0, mBloomColor6thVM));
...
texture DirtMap< string ResourceName = "dirtmap/Dirtmap.png"; >;
...
// 遮罩
texture LightBloom_Mask : OFFSCREENRENDERTARGET <
                     string Description = "the Mask information for LightBloom_SJ.x";
...
texture BloomMap6th : RENDERCOLORTARGET<float2 ViewportRatio={0.015625, 0.015625}; string Format="A16B16G16R16F";>;
...
texture BloomMap6thTemp : RENDERCOLORTARGET<float2 ViewportRatio={0.015625, 0.015625}; string Format="A16B16G16R16F";>;
...
sampler BloomSamp6th = sampler_state {
	texture = <BloomMap5th>;
	MinFilter = Linear; MagFilter = Linear; MipFilter = NONE;
	AddressU = BORDER; AddressV = BORDER; BorderColor = 0.0;
};
sampler BloomSamp6thTemp = sampler_state {
	texture = <BloomMap5thTemp>;
	MinFilter = Linear; MagFilter = Linear; MipFilter = NONE;
	AddressU = BORDER; AddressV = BORDER; BorderColor = 0.0;
};
...
float3 ColorSaturate(float3 color, float colorSaturation)
{
	...
	uniform sampler source) : COLOR
{
	...
	float3 mask = tex2Dlod(MaskMapSmp, float4(coord.xy, 0, 0)).rgb;

	float3 bloom = max(0, smoothstep(0, 1, color - mBloomThresholdP) * color);
	//float3 BloomStep = step(mBloomThresholdP, color);
	//float3 bloom = BloomStep * color + (1.0 - BloomStep) * smoothstep(mBloomThresholdP, 0, mBloomThresholdP - color) * color;
	bloom = ColorSaturate(bloom, size);
	bloom = bloom * mask;
...
float4 GlareLightCompVS(
	...
	out float3 oTexcoord6 : TEXCOORD6) : POSITION
{
	...
	float g6 = exp(-(4 * 4) / (2.0 * mBloomRadius * mBloomRadius));
	float weight = g1 + g2 + g3 + g4 + g5 + g6;
	...
	oTexcoord6 = g6 / weight * hsv2rgb(mBloomColor6th);
...
float4 GlareLightCompPS(
	...
	in float3 coord6 : TEXCOORD5,
	in float3 coord7 : TEXCOORD6) : COLOR
{
	...
	float3 bloom6 = tex2Dlod(BloomSamp6th, float4(coord1 + 0.5 / (ViewportSize * 0.015625), 0, 0)).rgb;
	...
	bloom *= hsv2rgb(mBloomColorAll) * 1.0;

	float3 dirt = tex2Dlod(DirtSampler, float4(coord1, 0, 0)).rgb;
	dirt = pow(dirt + 1e-8, 2.2);
	bloom += bloom * dirt * DirtIntensity * 128;
...
float4 HDRTonemappingPS(in float4 coord: TEXCOORD0, uniform sampler source) : COLOR
{
	...
	float3 curr = linear2srgb(color + bloom);
...
technique FimicGrain <
	...
	"RenderColorTarget=BloomMap6th;		 Pass=BloomDownsampleX6;"
	"RenderColorTarget=BloomMap6thTemp;  Pass=BloomBlurX6;"
	"RenderColorTarget=BloomMap6th;		 Pass=BloomBlurY6;"
...
;> {
	...
	pass BloomDownsampleX6<string Script= "Draw=Buffer;";>{
		AlphaBlendEnable = false; AlphaTestEnable = false;
		ZEnable = false; ZWriteEnable = false;
		VertexShader = compile vs_3_0 ScreenSpaceQuadOffsetVS(BloomOffset5);
		PixelShader  = compile ps_3_0 HDRDownsamplePS(BloomSamp5th);
	}
	pass BloomBlurX6<string Script= "Draw=Buffer;";>{
		AlphaBlendEnable = false; AlphaTestEnable = false;
		ZEnable = false; ZWriteEnable = false;
		VertexShader = compile vs_3_0 ScreenSpaceQuadOffsetVS(BloomOffset6);
		PixelShader  = compile ps_3_0 BloomBlurPS(BloomSamp6th, BloomOffsetX6);
	}
	pass BloomBlurY6<string Script= "Draw=Buffer;";>{
		AlphaBlendEnable = false; AlphaTestEnable = false;
		ZEnable = false; ZWriteEnable = false;
		VertexShader = compile vs_3_0 ScreenSpaceQuadOffsetVS(BloomOffset6);
		PixelShader  = compile ps_3_0 BloomBlurPS(BloomSamp6thTemp, BloomOffsetY6);
	}
